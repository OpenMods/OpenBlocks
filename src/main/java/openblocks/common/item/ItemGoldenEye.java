...
public class ItemGoldenEye extends Item {
	...
	private static void tryLearnStructure(ItemStack stack, WorldServer world, EntityPlayerMP player) {
		...
		if (!Strings.isNullOrEmpty(newStructureName) && newStructurePos != null) {
			player.addChatComponentMessage(new TextComponentTranslation("openblocks.misc.locked_on_nearest_structure", new TextComponentTranslation(StructureRegistry.structureNameLocalizationKey(newStructureName))));
