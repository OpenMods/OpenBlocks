...
public class ElevatorBlockRules {
	...
	private static List<IBlockState> parseBlockDesc(String blockDesc) {
		...
		if (blockDescParts.size() == 1) {
			...
			return decodeState(block, stateDesc);
		}
	}

	@SuppressWarnings("deprecation")
	private static List<IBlockState> decodeState(Block block, final String stateDesc) {
		final int meta = Integer.parseInt(stateDesc);
		return ImmutableList.of(block.getStateFromMeta(meta));
	}
