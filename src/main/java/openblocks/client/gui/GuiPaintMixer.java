...
public class GuiPaintMixer extends SyncedGuiContainer<ContainerPaintMixer> {
	...
	public GuiPaintMixer(ContainerPaintMixer container) {
		...
		root.addComponent(new GuiComponentRect(componentParent, 121, 74, 20, 20, CYAN));
		root.addComponent(new GuiComponentRect(componentParent, 141, 74, 20, 20, MAGENTA));
		root.addComponent(new GuiComponentRect(componentParent, 121, 94, 20, 20, YELLOW));
		root.addComponent(new GuiComponentRect(componentParent, 141, 94, 20, 20, KEY));

		{
			final GuiComponentLevel level = new GuiComponentLevel(componentParent, 118, 74 + 6, 2, 14, CYAN, 0xFF888888, 0f, 2f, 0);
			addSyncUpdateListener(ValueCopyAction.create(mixer.getDyeSlot(DyeSlot.cyan), level));
			root.addComponent(level);
		}

		{
			final GuiComponentLevel level = new GuiComponentLevel(componentParent, 141 + 21, 74 + 6, 2, 14, MAGENTA, 0xFF888888, 0f, 2f, 0);
			addSyncUpdateListener(ValueCopyAction.create(mixer.getDyeSlot(DyeSlot.magenta), level));
			root.addComponent(level);
		}

		{
			final GuiComponentLevel level = new GuiComponentLevel(componentParent, 118, 94 + 6, 2, 14, YELLOW, 0xFF888888, 0f, 2f, 0);
			addSyncUpdateListener(ValueCopyAction.create(mixer.getDyeSlot(DyeSlot.yellow), level));
			root.addComponent(level);
		}

		{
			final GuiComponentLevel level = new GuiComponentLevel(componentParent, 141 + 21, 94 + 6, 2, 14, KEY, 0xFF888888, 0f, 2f, 0);
			addSyncUpdateListener(ValueCopyAction.create(mixer.getDyeSlot(DyeSlot.black), level));
			root.addComponent(level);
		}

		{
			GuiComponentProgress progress = new GuiComponentProgress(componentParent, 125, 43, TileEntityPaintMixer.PROGRESS_TICKS);
			addSyncUpdateListener(ValueCopyAction.create(mixer.getProgress(), progress.progressReceiver()));
			root.addComponent(progress);
		}

		{
			GuiComponentTextButton buttonMix = new GuiComponentTextButton(componentParent, 125, 57, 30, 13, 0xFFFFFF);
		...
		{
			final GuiComponentTextbox textbox = new GuiComponentTextbox(componentParent, 65, 90, 44, 10);
			root.addComponent(textbox);

			final GuiComponentColorPicker colorPicker = new GuiComponentColorPicker(componentParent, 10, 20);
			root.addComponent(colorPicker);

			final GuiComponentSlider slider = new GuiComponentSlider(componentParent, 10, 75, 100, 0, 255, 0, false);
			root.addComponent(slider);

			final GuiComponentRect colorBox = new GuiComponentRect(componentParent, 10, 90, 45, 10, 0xFFFFFF);
			root.addComponent(colorBox);

			final GuiComponentPalettePicker palettePicker = new GuiComponentPalettePicker(componentParent, 112, 25);
