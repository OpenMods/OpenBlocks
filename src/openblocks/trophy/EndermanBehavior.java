    ...
    protected void teleportTo(TileEntity tile, EntityPlayer player, double par1, double par3, double par5)
    {
        ...
        else
        {
            ...
            for (l = 0; l < short1; ++l)
            {
                ...
                double d7 = d3 + (player.posX - d3) * d6 + (tile.worldObj.rand.nextDouble() - 0.5D) * (double)player.width * 2.0D;
                double d8 = d4 + (player.posY - d4) * d6 + tile.worldObj.rand.nextDouble() * (double)player.height;
                double d9 = d5 + (player.posZ - d5) * d6 + (tile.worldObj.rand.nextDouble() - 0.5D) * (double)player.width * 2.0D;
