...
public class PacketHandler implements IPacketHandler {
	...
	public void onPacketData(INetworkManager manager,
		...
		if (packet.channel.equals("OpenBlocks")) {
			try {
				...
				Log.warn(e, "Error while handling data from player '%s'",
						player);
			}
		}
	}

	public static List<EntityPlayer> getPlayersInRange(World world, int blockX,
			int blockZ, int distance) {
	...
	public static boolean shouldSendUpdateToPlayer(World worldObj, int sourceX,
			int sourceZ, Player player) {
		/* Nope for clients */
		...
		if (worldObj.getChunkProvider().chunkExists(sourceX, sourceZ)) {
			System.out
					.println("Chunk doesn't exist " + sourceX + "," + sourceZ);
			return false;
		}
	...
	public static List<EntityPlayer> getPlayersThatNeedUpdates(World worldObj,
		...
		while (it.hasNext()) {
			...
			if (ent instanceof EntityPlayerMP) {
				...
				if (mpInstance.loadedChunks != null
						&& !mpInstance.loadedChunks.isEmpty()) {
					@SuppressWarnings("unchecked")
					Iterator<Chunk> iterator = mpInstance.loadedChunks
							.iterator();
					while (iterator.hasNext()) {
						Chunk testChunk = iterator.next();
						if (testChunk == chunk) {
