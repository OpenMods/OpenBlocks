...
public class ContainerLightbox extends Container {
	...
	protected void addInventoryGrid(int xOffset, int yOffset, int width) {
		...
		for (int y = 0, slotId = 0; y < width; y++) {
			for (int x = 0; x < height; x++, slotId++) {
				System.out.println("Adding slot " + slotId);
				addSlotToContainer(new Slot(lightbox, slotId, xOffset + x * 18, yOffset + y * 18));
			}
		}
	}
	*/
	...
	protected void addPlayerInventorySlots(int offsetY) {
		for (int l = 0; l < 3; l++) {
			for (int k1 = 0; k1 < 9; k1++) {
				addSlotToContainer(new Slot(playerInventory, k1 + l * 9 + 9, 8 + k1 * 18, offsetY + l * 18));
			}
		}
