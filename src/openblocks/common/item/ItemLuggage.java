...
public class ItemLuggage extends Item {
	...
	public ItemStack onItemRightClick(ItemStack itemStack, World world,
		...
		if (!world.isRemote) {
			...
			Vec3 vec3 = world.getWorldVec3Pool().getVecFromPool(player.posX,
					player.posY, player.posZ);
			Vec3 vec31 = player.getLook(1.0f);
			Vec3 vec32 = vec3.addVector(vec31.xCoord * 2.0f,
					vec31.yCoord * 2.0f, vec31.zCoord * 2.0f);
			EntityLuggage luggage = new EntityLuggage(world);
			luggage.setPositionAndRotation(0.5 + vec32.xCoord, vec3.yCoord,
					0.5 + vec32.zCoord, 0, 0);
