...
public class ShapeCuboidGenerator implements IShapeGenerator {
	...
	public void generateShape(int xSize, int ySize, int zSize, IShapeable shapeable) {
		...
		/*
		 * Optimization: The front and back planes are full sized, left and right are 2 blocks smaller
		 * so that they don't overlap the front and back planes. Top and bottom planes are 2 blocks smaller
		 * in both directions so they don't overlap any sides. This works for most cases unless two directions
		 * are the same because of rounding in small numbers.
		 * 
		 * But this should be fairly optimal without adding anything inefficient to complicate matters.
		 * - NeverCast
		 */

		/* Used for shrinking some planes to prevent iterating the same block */
		/* Basically, the size is size * 2 - 2 unless that's less than 1 and size is > 0 */
